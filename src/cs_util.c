...
int
cs_set_linger(int sd, int timeout)
{
...
int
_cs_atoi(uint8_t *line, size_t n)
{
    int value;
...
void *
_cs_alloc(size_t size, const char *name, int line)
{
    void *p;
...
void *
_cs_zalloc(size_t size, const char *name, int line)
{
    void *p;

    p = _cs_alloc(size, name, line);
    if (p != NULL) {
        memset(p, 0, size);
    }
...
void *
_cs_calloc(size_t nmemb, size_t size, const char *name, int line)
{
    return _cs_zalloc(nmemb * size, name, line);
}

void *
_cs_realloc(void *ptr, size_t size, const char *name, int line)
{
    void *p;
...
void
_cs_free(void *ptr, const char *name, int line)
{
    ASSERT(ptr != NULL);
...
void
cs_assert(const char *cond, const char *file, int line, int panic)
{
 ...
 */
ssize_t
_cs_sendn(int sd, const void *vptr, size_t n)
{
    size_t nleft;
    ssize_t	nsend;
    const char *ptr;
 ...
 */
ssize_t
_cs_recvn(int sd, void *vptr, size_t n)
{
	size_t nleft;
	ssize_t	nrecv;
	char *ptr;
...
cs_msec_now(void)
{
    return cs_usec_now() / 1000LL;
}

static int
cs_resolve_inet(struct string *name, int port, struct sockinfo *si)
{
    int status;
    struct addrinfo *ai, *cai; /* head and current addrinfo */
    struct addrinfo hints;
    char *node, service[M_UINTMAX_MAXLEN];
    bool found;

    ASSERT(cs_valid_port(port));
    ...
    cs_snprintf(service, M_UINTMAX_MAXLEN, "%d", port);
    ...
    for (cai = ai, found = false; cai != NULL; cai = cai->ai_next) {
        ...
        cs_memcpy(&si->addr, cai->ai_addr, si->addrlen);
        found = true;
        break;
    }
...
static int
cs_resolve_unix(struct string *name, struct sockinfo *si)
{
    struct sockaddr_un *un;

    if (name->len >= M_UNIX_ADDRSTRLEN) {
        return -1;
    }
    ...
    cs_memcpy(un->sun_path, name->data, name->len);
 ...
 */
int
cs_resolve(struct string *name, int port, struct sockinfo *si)
{
    if (name != NULL && name->data[0] == '/') {
        return cs_resolve_unix(name, si);
    }

    return cs_resolve_inet(name, port, si);
}
 ...
 */
char *
cs_unresolve_addr(struct sockaddr *addr, socklen_t addrlen)
{
    ...
    cs_snprintf(unresolve, sizeof(unresolve), "%s:%s", host, service);
...
cs_unresolve_peer_desc(int sd)
{
    ...
    return cs_unresolve_addr(addr, addrlen);
}
